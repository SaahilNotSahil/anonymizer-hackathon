...
def parse_args():
    ...
    parser.add_argument(
        ...
        dest='input',
        help='''
        inputfile1:columnindex1 [inputfile2:columnindex2] for csv or
        inputfile1:./xpath-selector/@attribute_name for xml, use multiple
        arguments to replace in multiple files!
        '''
    )
    parser.add_argument(
        '-t',
        ...
        default='number',
        help='''
        name, first_name, last_name, email, zip, city, address, number, ... .
        Default is number
        '''
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=';',
        help='''
        the delimiter of the csv columns. For tab as delimiter use
    ...
    parser.add_argument(
        ...
        default='0',
        help='''
        set to number of header lines in csv files to ignore, default = 0
        '''
    )
    parser.add_argument(
        '--namespace',
        ...
        dest='namespace',
        help='''
        shortname=http://full-url-of-namespace.com add xml namespaces so they
        can be used in xpath selector, sepearate with equals
        '''
    )
    ...
    for row in rows:
        ...
        if len(row) > column_index:
            if len(row[column_index].strip()) > 0:
                row[column_index] = FAKE_DICT[row[column_index].strip()
                                              .replace('\n', '')]
...
def anonymize_csv(
    ...
    with open(source_file_name, 'r', encoding=encoding, newline=None) \
            as inputfile:
...
def anonymize_xml(
    source_file_name, target_file_name, selector, encoding, namespaces
):
    """
    ...
    The selector is an xpath string to determine which element/attribute to
    anonymize.
    """
...
if __name__ == '__main__':
    ...
    # special handling for tab delimiter to allow easier passing as command
    # line:
    if ARGS.delimiter == "\t":
    ...
    for infile in ARGS.input:
        ...
        for extendedFile in files_to_read:
            ...
            if os.path.isfile(source):
                ...
                if ARGS.overwrite:
                    print(
                        'overwriting original file %s with anonymized file!' %
                        source
                    )
